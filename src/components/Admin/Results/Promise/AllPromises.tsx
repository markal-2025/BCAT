...
const AllPromises = () => {
  ...
  const { getPromises, promises } = useAuth();

  useEffect(() => {
    if (teamId) {
      getPromises(Number(teamId));
    }
  }, [teamId]);
  ...
  const handleViewPromises = (promiseSessionId: number) => {
    navigate(
      `/organizations/${organizationId}/teams/${teamId}/promises/${promiseSessionId}`
    );
  };
  ...
  return promises ? (
    <div className="p-2">
      <div className="flex flex-col items-start justify-between mt-8 mb-8 sm:flex-row sm:items-center">
        <h1 className="mb-4 text-2xl font-medium sm:mb-0">Promise Sessions</h1>
      </div>
          ...
          >
            ...
            <div className="flex flex-col col-span-5 md:hidden">
              <div className="font-medium">
                ...
                {promise.team?.name || "N/A"}
              </div>
              <div className="text-sm text-gray-600">
                {promise.promise?.organization?.name || "N/A"}
              </div>
            ...
            <div className="hidden col-span-2 font-medium md:block">
              {promise.team?.name || "N/A"}
            </div>
            <div className="hidden col-span-2 text-gray-600 md:block">
              {promise.promise?.organization?.name || "N/A"}
            </div>
